...
En esta sección se presentan los principales hallazgos obtenidos en la comparación de estrategias de Control Basado en Eventos (CBE). Se incorporan los resultados tanto de simulaciones como de implementaciones físicas para analizar el rendimiento de cada estrategia.

Los resultados de simulación e implementación del método SoD muestran que la variación del parámetro \textbf{\textit{elim}} tiene un impacto directo en el número de eventos generados. La Figura \ref{fig:figura2} presenta la respuesta temporal y el número acumulado de eventos en el tiempo bajo simulación, mientras que la Figura \ref{fig:figura4} muestra los resultados obtenidos en la implementación física con PLCs, donde se observa una correspondencia mejorada tras considerar la saturación y zona muerta de los actuadores.

Se han identificado diferencias entre simulación e implementación que inicialmente dificultaron la validación de resultados. Sin embargo, la incorporación de modelos de saturación y zona muerta permitió que los resultados físicos y simulados converjan de manera más efectiva. Este ajuste fue producto de un proceso iterativo basado en heurística y experiencia, que permitió refinar el procedimiento y sistematizarlo progresivamente para facilitar su replicación.
...
Además, se realizó el cálculo correspondiente para estrategias periódicas, y mediante diferencias porcentuales se determinó la eficiencia relativa de las estrategias CBE frente al control periódico.

Un desafío particular fue la implementación de estrategias como Lyapunov, donde el algoritmo tuvo que generarse desde cero a partir de la teoría. Asimismo, al trasladar los algoritmos a los PLCs, se enfrentaron restricciones como la falta de funciones matemáticas complejas (por ejemplo, \texttt{exp}), lo que obligó a usar aproximaciones como series de Taylor para su implementación efectiva.

Este procedimiento, aunque centrado en la comparación, se complementa con una metodología de despliegue independiente desarrollada por Karel Walter Gómez Orellana titulada \textit{"Metodología de despliegue de sistemas de control basado en eventos para procesos con infraestructura de control compartida o limitada"} \cite{Gomez2025}. Mientras que la presente contribución permite seleccionar la estrategia más adecuada para un sistema dado, dicha metodología aborda los pasos necesarios para su integración operativa.

\section{Conclusiones}
La aplicación del procedimiento de benchmarking desarrollado demuestra que las estrategias de CBE representan una alternativa viable y eficiente frente al control periódico en entornos industriales con recursos compartidos. La reducción en la cantidad de activaciones del actuador permite extender la vida útil de los dispositivos y mejorar la eficiencia al reducir los cálculos computacionales sin afectar significativamente el rendimiento del sistema.

Se concluye que la estrategia SoD es particularmente adecuada para aplicaciones donde la optimización del consumo de recursos computacionales es prioritaria, manteniendo un desempeño comparable al CP. Sin embargo, su implementación requiere un ajuste adecuado de los parámetros de control y de la condición de evento para garantizar su efectividad en diferentes escenarios operativos.
...
A pesar de los resultados positivos, este estudio presenta algunas limitaciones. El procedimiento parte del supuesto de que el sistema ya cuenta con un controlador periódico implementado, lo cual restringe su aplicación directa en entornos sin dicha base. Asimismo, no constituye una metodología de despliegue de controladores basada en eventos, enfoque que ha sido desarrollado paralelamente por Karel Walter Gómez Orellana y complementa este trabajo desde una perspectiva operativa.
...
La aplicación del CBE en la industria tiene un gran potencial para optimizar el consumo de recursos computacionales y extender la vida útil de los actuadores. Este procedimiento de benchmarking proporciona una metodología práctica, sistematizada y replicable para evaluar la viabilidad de estas estrategias en distintos entornos industriales.
...
\bibitem{Gomez2025}
K. W. Gómez Orellana, \textit{Metodología de despliegue de sistemas de control basado en eventos para procesos con infraestructura de control compartida o limitada}, Universidad Mayor de San Andrés, 2024.
\end{thebibliography}
